...
object Detekto {
  ...
  def createService[F[_]: LogIO: Timer: ConcurrentEffect: ContextShift](
    ...
    for {
      ...
      srv  <- create(http, token, conf, ec, dmCodeRepo)
    } yield srv
  }
  ...
  private def create[F[_]: Timer: ConcurrentEffect : ContextShift: LogIO](
    ...
    for {
      bot <- createBot(httpWithLog, token, conf, dmCodeRepo)
    } yield new Service[F] {
  ...
  private def createBot[F[_]: Api: Timer: ConcurrentEffect : LogIO](
    ...
    Resource.make(F.delay(new Bot[F](http, token, conf, dmCodeRepo)))(_ => F.unit)
  }
